...
| [**Pwnagotchi**](https://github.com/evilsocket/pwnagotchi) | A2C "AI" driving bettercap to capture WPA handshakes/PMKIDs | Pi Zero W + e-ink | Python | **GPL-3.0** ⚠️ | **mood → face state machine** (→ our `bear_state`); **plugin/event hooks** (`on_*`) → our detector model; `config.toml` personality; bettercap-as-sensor → our Pi capture |
...
| [**Bruce**](https://github.com/pr3y/Bruce) | "Predatory" multi-tool ESP32 firmware (WiFi/BLE/RF/IR/RFID) | M5Core/Cardputer/Stick/T-Deck | C++/Arduino | **AGPL-3.0** ⚠️ | M5 **menu/navigation architecture**; per-device build matrix; config in SPIFFS/LittleFS; WebUI idea (later) |
